...
class Token(tuple):
    ...
    def __repr__(self):
        return 'Token(%r, %r, %r)' % (self.type, self.value, self.pos)
    ...
    def expect(self, expr):
        ...
        if not self.current.test(expr):
            ...
            raise ExpressionSyntaxError(
                ...
                (expr, describe_token(self.current)), self.current.pos)
        try:
            return self.current
        finally:
            next(self)
...
class Lexer(object):
    ...
    def tokeniter(self, code):
            ...
            if kind == "SKIP":
                ...
                if value == '(':
                    ...
                    if not balancing_stack:
                        raise ExpressionSyntaxError(
                            f'{value!r} unexpected on pos {pos}')
                    expected_op = balancing_stack.pop()
                    if expected_op != value:
                        raise ExpressionSyntaxError(
                            f'{value!r} unexpected on pos {pos}, excepted {expected_op!r}'
                        )
            elif kind == "MISMATCH":
                # 不支持识别的字符或字符串直接抛出错误
                raise ExpressionSyntaxError(
        ...
        if balancing_stack:
            raise ExpressionSyntaxError(
                f'expected {balancing_stack[0]!r} on pos {pos}')
